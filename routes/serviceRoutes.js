...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!phoneNumber || !ISD) {
      return res
        .status(400)
        .json({ message: 'Phone number and ISD code are required.' });
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!phoneNumber || !otpCode || !serviceId) {
      return res
        .status(400)
        .json({
          message: 'Phone number, OTP code, and service ID are required.',
        });
    }
...
router.post(
  '/service/:id/follow-up',
  createAndUpdateService,
  followUpServiceData
);

router.get('/service/inquire/:code', inquireServiceByCode);
